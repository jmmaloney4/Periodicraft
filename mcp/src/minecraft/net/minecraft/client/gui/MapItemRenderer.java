    ...
    public void renderMap(EntityPlayer par1EntityPlayer, RenderEngine par2RenderEngine, MapData par3MapData)
    {
        for (int i = 0; i < 16384; ++i)
        {
            ...
            else
            {
                ...
                {
                    int k1 = (l * 30 + i1 * 59 + j1 * 11) / 100;
                    int l1 = (l * 30 + i1 * 70) / 100;
                    int i2 = (l * 30 + j1 * 70) / 100;
                    l = k1;
                    i1 = l1;
                    j1 = i2;
                }
        ...
        for (Iterator iterator = par3MapData.playersVisibleOnMap.values().iterator(); iterator.hasNext(); ++j2)
        {
            ...
            tessellator.startDrawingQuads();
            tessellator.addVertexWithUV(-1.0D, 1.0D, (double)((float)j2 * 0.001F), (double)f1, (double)f2);
            tessellator.addVertexWithUV(1.0D, 1.0D, (double)((float)j2 * 0.001F), (double)f3, (double)f2);
            tessellator.addVertexWithUV(1.0D, -1.0D, (double)((float)j2 * 0.001F), (double)f3, (double)f4);
            tessellator.addVertexWithUV(-1.0D, -1.0D, (double)((float)j2 * 0.001F), (double)f1, (double)f4);
            tessellator.draw();
            GL11.glPopMatrix();
        }
